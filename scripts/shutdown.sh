...
# Graceful shutdown + optional backup (minimal)
...
echo "➡️ Sauvegarde base (si en cours)..."
if $DC ps postgres | grep -q Up; then
  if $DC exec -T postgres pg_isready -U scrappingbot_user >/dev/null 2>&1; then
    $DC exec -T postgres pg_dump -U scrappingbot_user scrappingbot >"$backup_dir/postgres_${timestamp}.sql" || true
  fi
fi

echo "➡️ Arrêt des services"
$DC down --remove-orphans
echo "✅ Shutdown terminé"#!/bin/bash
